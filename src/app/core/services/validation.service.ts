...
export class ValidationService {
  ...
  sanitizeVideoId(videoId: string): string {
    const clean = videoId.replace(/[^a-zA-Z0-9_-]/g, '');
    return validateVideoId(clean) ? clean : '';
  }

  isValidTimeRange(start: number, end: number, duration: number): boolean {
    const result = isValidTimeRange(start, end, duration);
  ...
  parseTime(timeString: string): number {
    return parseTimeString(timeString) || 0;
  }
